...
import { ActionsListContainer, ActionsListButton, ActionsListPanel, ScrollableOptimisationsContainer, OptimisationTemplateContainer, 
TypeText, TrashIcon, GoToIcon, TimeIcon, TotalContainer, TotalTitleText, GaugesContainer } from './elements';
...
const ActionsList: React.FC<ActionsListProps> = ({ id, isDark, actionsListExtended, setActionsListExtended, 
  decisionPanelExtended, optimisationTemplateData, setOptimisationTemplateData }) => {
        ...
        {optimisationTemplateData
          ...
          .map((item: any, i: number) => (
            ...
            >
              ...
              <TrashIcon isDark={isDark} size={30} onClick={() => handleTrashIconClick(item.id)}></TrashIcon>
              <GoToIcon isDark={isDark} size={30}></GoToIcon>
            </OptimisationTemplateContainer>
          ))}
        </ScrollableOptimisationsContainer>
          <TotalContainer isDark={isDark}>
            <TotalTitleText isDark={isDark}>Impact Économique</TotalTitleText>
          </TotalContainer>
          {/* Render personalized gauge components */}
          <GaugesContainer>
            <PersonnalizedGauge id={"ElecGaugesComponentId"} isDark={isDark} isElec={true} isBio={false} isLumi={false} level={80} oldLevel={50} top={315} left={470} />
            <PersonnalizedGauge id={"BioGaugesComponentId"} isDark={isDark} isElec={false} isBio={true} isLumi={false} level={65} oldLevel={85} top={315} left={570} />
            <PersonnalizedGauge id={"LumiGaugesComponentId"} isDark={isDark} isElec={false} isBio={false} isLumi={true} level={40} oldLevel={20} top={315} left={670} />
          </GaugesContainer>
      </ActionsListPanel>
    </ActionsListContainer>
  )
