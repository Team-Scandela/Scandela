...
async function fetchRealTimeTrafficData(): Promise<any> {
    ...
    try {
        const response = await fetch(
            `https://api.mapbox.com/mapbox/driving-traffic/v1/mapbox/driving-traffic/v1/?access_token=${accessToken}`
        );

        if (!response.ok) {
            throw new Error(
                `Failed to fetch real-time traffic data. Status: ${response.status}`
            );
        }
...
function filterTrafficDataForFrance(trafficData: any): any {
    ...
    const franceBounds = [
        [-5.559374, 41.33374], // Sud-ouest de la France
...
async function fetchLightningData(): Promise<any> {
    try {
        ...
        if (!response.ok) {
            throw new Error(
                `Failed to fetch lightning data. Status: ${response.status}`
            );
        }
