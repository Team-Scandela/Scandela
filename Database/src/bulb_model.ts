...
const createBulb = (request : Request, response : Response) => {
    ...
    const query = 'INSERT INTO bulb (uuid, reference, intensity, consumption, type) VALUES (CAST($1 AS uuid), $2, $3, $4, $5) RETURNING uuid';
    ...
    pool.query(query, values, (error : any, results : any) => {
        ...
        response.status(201).send(`Bulb added with ID: ${results.rows[0].uuid}`);
    });
}
...
const deleteBulb = (request : Request, response : Response) => {
    ...
    pool.query('DELETE FROM bulb WHERE uuid = CAST($1 AS uuid)', [uuid], (error : any, results : any) => {
        if (error) {
            throw error;
        }
...
const updateBulb = (request : Request, response : Response) => {
    ...
    const query = 'UPDATE bulb SET reference = $1, intensity = $2, consumption = $3, type = $4 WHERE uuid = CAST($5 AS uuid)';
