...
const createLampshade = (request : Request, response : Response) => {
    ...
    const query = 'INSERT INTO lampshade (uuid, adress, uuidhood) VALUES (CAST($1 AS uuid), $2, CAST($3 AS uuid)) RETURNING uuid';
...
const updateLampshade = (request : Request, response : Response) => {
    ...
    const query = 'UPDATE lampshade SET adress = $1, uuidhood = CAST($2 AS uuid) WHERE uuid = CAST($3 AS uuid)';
