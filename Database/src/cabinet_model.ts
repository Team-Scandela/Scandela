...
const createCabinet = (request : Request, response : Response) => {
    ...
    const query = 'INSERT INTO cabinet (uuid, reference, lat, lng) VALUES (CAST($1 AS uuid), $2, $3, $4) RETURNING uuid';
    ...
    pool.query(query, values, (error : any, results : any) => {
        ...
        response.status(201).send(`Cabinet added with ID: ${results.rows[0].uuid}`);
    });
}
...
const deleteCabinet = (request : Request, response : Response) => {
    ...
    pool.query('DELETE FROM cabinet WHERE uuid = CAST($1 AS uuid)', [uuid], (error : any, results : any) => {
        if (error) {
            throw error;
        }
...
const updateCabinet = (request : Request, response : Response) => {
    ...
    const query = 'UPDATE cabinet SET reference = $1, lat = $2, lng = $3 WHERE uuid = CAST($4 AS uuid)';
