...
const createDecisiontype = (request: Request, response: Response) => {
    ...
    if (title == null) throw new Error('title must be defined');

    const query =
        'INSERT INTO decisiontype (uuid, title, moreinfo) VALUES (CAST($1 AS uuid), $2, $3) RETURNING uuid';
    ...
    pool.query(query, values, (error: any, results: any) => {
        ...
        response
            .status(201)
            .send(`Decisiontype added with ID: ${results.rows[0].uuid}`);
    });
};
...
const deleteDecisiontype = (request: Request, response: Response) => {
    ...
    pool.query(
        'DELETE FROM decisiontype WHERE uuid = CAST($1 AS uuid)',
        [uuid],
...
const updateDecisiontype = (request: Request, response: Response) => {
    ...
    const query =
        'UPDATE decisiontype SET title = $1, moreinfo = $2 WHERE uuid = CAST($3 AS uuid)';
