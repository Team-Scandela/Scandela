...
const createHood = (request: Request, response: Response) => {
    ...
    const query =
        'INSERT INTO hood (uuid, name, lat, lng, uuidtown) VALUES (CAST($1 AS uuid), $2, $3, $4, CAST($5 AS uuid)) RETURNING uuid';
    ...
    pool.query(query, values, (error: any, results: any) => {
        ...
        response
            .status(201)
            .send(`Hood added with ID: ${results.rows[0].uuid}`);
    });
};
...
const deleteHood = (request: Request, response: Response) => {
    ...
    pool.query(
        'DELETE FROM hood WHERE uuid = CAST($1 AS uuid)',
        [uuid],
...
const updateHood = (request: Request, response: Response) => {
    ...
    const query =
        'UPDATE hood SET name = $1, lat = $2, lng = $3, uuidtown = CAST($4 AS uuid) WHERE uuid = CAST($5 AS uuid)';
