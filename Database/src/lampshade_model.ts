...
const createLampshade = (request: Request, response: Response) => {
    ...
    const query =
        'INSERT INTO lampshade (uuid, reference, quality) VALUES (CAST($1 AS uuid), $2, $3) RETURNING uuid';
...
const updateLampshade = (request: Request, response: Response) => {
    ...
    const query =
        'UPDATE lampshade SET reference = $1, quality = $2 WHERE uuid = CAST($3 AS uuid)';
