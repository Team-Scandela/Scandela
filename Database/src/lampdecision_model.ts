...
const createLampdecision = (request : Request, response : Response) => {
    ...
    const query = 'INSERT INTO lampdecision (uuid, uuidlamp, uuiddecision) VALUES (CAST($1 AS uuid), CAST($2 AS uuid), CAST($3 AS uuid)) RETURNING uuid';
    ...
    pool.query(query, values, (error : any, results : any) => {
        ...
        response.status(201).send(`Lampdecision added with ID: ${results.rows[0].uuid}`);
    });
}
...
const deleteLampdecision = (request : Request, response : Response) => {
    ...
    pool.query('DELETE FROM lampdecision WHERE uuid = CAST($1 AS uuid)', [uuid], (error : any, results : any) => {
        if (error) {
            throw error;
        }
...
const updateLampdecision = (request : Request, response : Response) => {
    ...
    const query = 'UPDATE lampdecision SET uuidlamp = CAST($1 AS uuid), uuiddecision = CAST($2 AS uuid) WHERE uuid = CAST($3 AS uuid)';
