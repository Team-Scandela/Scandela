...
const createTown = (request: Request, response: Response) => {
    const { name, lat, lng, electrictyprice, eleclevel, ecolevel, qualilevel } =
        request.body;

    if (name == null) throw new Error('name must be defined');

    const query =
        'INSERT INTO town (uuid, name, lat, lng, electrictyprice, eleclevel, ecolevel, qualilevel) VALUES (CAST($1 AS uuid), $2, $3, $4, $5, $6, $7, $8) RETURNING uuid';
    const values = [
        uuidv4(),
        name,
        lat,
    ...
    pool.query(query, values, (error: any, results: any) => {
        ...
        response
            .status(201)
            .send(`Town added with ID: ${results.rows[0].uuid}`);
    });
};
...
const deleteTown = (request: Request, response: Response) => {
    ...
    pool.query(
        'DELETE FROM town WHERE uuid = CAST($1 AS uuid)',
        [uuid],
...
const updateTown = (request: Request, response: Response) => {
    ...
    const query =
        'UPDATE town SET name = $1, lat = $2, lng = $3, electrictyprice = $4, eleclevel = $5, ecolevel = $6, qualilevel = $7 WHERE uuid = CAST($8 AS uuid)';
    const values = [
        name,
        lat,
        lng,
